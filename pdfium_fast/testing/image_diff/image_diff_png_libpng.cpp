...
#ifdef HAS_NEON_SIMD
  // Process 16 pixels (64 bytes) at a time with NEON
  // vld4q_u8 loads 4 channels × 16 elements = 64 bytes (16 BGRA pixels)
  for (; x + 15 < pixel_width; x += 16) {
    uint8x16x4_t channels = vld4q_u8(input + x * 4);
    uint8x16x4_t result;
    result.val[0] = channels.val[2];  // R (was B)
    result.val[1] = channels.val[1];  // G (unchanged)
    result.val[2] = channels.val[0];  // B (was R)
    result.val[3] = channels.val[3];  // A (unchanged)
    vst4q_u8(output + x * 4, result);
  }

  // Process 8 pixels (32 bytes) at a time for remaining
  // vld4_u8 loads 4 channels × 8 elements = 32 bytes (8 BGRA pixels)
  for (; x + 7 < pixel_width; x += 8) {
    uint8x8x4_t channels = vld4_u8(input + x * 4);
    uint8x8x4_t result;
    result.val[0] = channels.val[2];  // R (was B)
    result.val[1] = channels.val[1];  // G (unchanged)
    result.val[2] = channels.val[0];  // B (was R)
    result.val[3] = channels.val[3];  // A (unchanged)
    vst4_u8(output + x * 4, result);
  }
#elif defined(HAS_AVX2_SIMD)
  // Process 8 pixels (32 bytes) at a time with AVX2
  // Shuffle mask to swap R and B channels: [2,1,0,3, 6,5,4,7, 10,9,8,11, 14,13,12,15, ...]
  const __m256i shuffle_mask = _mm256_setr_epi8(
      2,1,0,3,  6,5,4,7,  10,9,8,11,  14,13,12,15,  // First 16 bytes
      2,1,0,3,  6,5,4,7,  10,9,8,11,  14,13,12,15   // Second 16 bytes
  );

  for (; x + 7 < pixel_width; x += 8) {
    __m256i pixels = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(input + x * 4));
    __m256i swapped = _mm256_shuffle_epi8(pixels, shuffle_mask);
    _mm256_storeu_si256(reinterpret_cast<__m256i*>(output + x * 4), swapped);
  }
#endif
...
bool DoLibpngWrite(png_struct* png_ptr,
  ...
  // Disable PNG filtering for maximum speed (N=225 optimization)
...
std::vector<uint8_t> Encode(pdfium::span<const uint8_t> input,
                            ...
                            bool discard_transparency,
                            const std::vector<Comment>& comments) {
  // Use Z_NO_COMPRESSION for maximum speed (N=225 optimization)
  // Profiling showed PNG encoding is 97% of rendering time
  return EncodeWithCompressionLevel(input, format, width, height,
                                    row_byte_width, discard_transparency,
                                    comments, Z_NO_COMPRESSION);
}
