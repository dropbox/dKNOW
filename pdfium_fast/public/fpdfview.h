...
#endif  // defined(WIN32)
#else
// Monolithic build: still export symbols when building the library
#if defined(FPDF_IMPLEMENTATION)
#if defined(WIN32)
#define FPDF_EXPORT __declspec(dllexport)
#else
...
// Enable GPU acceleration for rendering (macOS Metal backend).
// Note: Realistic speedup is 1.0-1.3x. Primary benefits are power efficiency
// and CPU offloading. Falls back to CPU if GPU is unavailable.
#define FPDF_GPU_ACCELERATED 0x8000
