...
// Declare external C functions from the bridge
extern "C" {
...
fn write_ppm(path: &Path, width: i32, height: i32, rgb_data: &[u8]) -> std::io::Result<()> {
    ...
    // P6 header: magic number, width, height, max color value
    // Match upstream format exactly (includes comment line)
    write!(writer, "P6\n# PDF test render\n{} {}\n255\n", width, height)?;
...
fn render_page(
    ...
    unsafe {
        ...
        let result = pdfium_bridge_render_page(c_path.as_ptr(), page_index, scale);
        ...
        println!(
            "Rendered page {} -> {} ({}x{} @ scale={})",
            page_index, output_path, width, height, scale
        );
...
fn main() {
    ...
    if args.len() < 5 {
        eprintln!(
            "Usage: {} <pdf_path> <output_dir> <page_index> <scale>",
            args[0]
        );
        eprintln!("Example: {} input.pdf /tmp/output 10 4.166666", args[0]);
        eprintln!("Note: For 300 DPI, use scale=4.166666 (matches upstream)");
        std::process::exit(1);
    }
    ...
    let scale: f64 = args[4].parse().unwrap_or_else(|_| {
        eprintln!("Invalid scale: {}", args[4]);
        std::process::exit(1);
    });
    ...
    match render_page(pdf_path, page_index, &output_path, scale) {
        Ok(_) => {
            println!("Success!");
        }
        Err(e) => {
