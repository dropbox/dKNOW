// Test with proper FFI callbacks
use pdfium_sys::*;
use std::ffi::CString;
use std::fs::File;
use std::io::Write;

// Global document handle for callback access
static mut GLOBAL_DOC: FPDF_DOCUMENT = std::ptr::null_mut();

// FFI callback: Get page by index
#[allow(non_snake_case)]
unsafe extern "C" fn ffi_get_page(
    _pThis: *mut FPDF_FORMFILLINFO,
    _document: FPDF_DOCUMENT,
    page_index: i32,
) -> FPDF_PAGE {
    // Load and return the requested page
    FPDF_LoadPage(GLOBAL_DOC, page_index)
}

// FFI callback: Execute named action (just log it)
#[allow(non_snake_case)]
unsafe extern "C" fn ffi_execute_named_action(
    _pThis: *mut FPDF_FORMFILLINFO,
    named_action: FPDF_BYTESTRING,
) {
    // Just acknowledge, don't need to do anything
    if !named_action.is_null() {
        eprintln!("[DEBUG] Named action called");
    }
}
...
fn main() {
    unsafe {
        ...
        // Store doc in global for callback access
        GLOBAL_DOC = doc;

        // Create form callbacks WITH function pointers
        let mut form_callbacks: FPDF_FORMFILLINFO = std::mem::zeroed();
        form_callbacks.version = 2;

        // SET THE CALLBACK FUNCTIONS (this is what's missing!)
        form_callbacks.FFI_GetPage = Some(ffi_get_page);
        form_callbacks.FFI_ExecuteNamedAction = Some(ffi_execute_named_action);
        ...
        println!("Page 10 with callbacks: {}x{} px", width_px, height_px);
        ...
        let mut file = File::create("/tmp/with_callbacks_page10.ppm").unwrap();
        write!(
            file,
            "P6\n# PDF test render\n{} {}\n255\n",
            width_px, height_px
        ...
        println!("Wrote /tmp/with_callbacks_page10.ppm");
