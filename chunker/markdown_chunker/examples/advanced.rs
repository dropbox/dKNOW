...
"#;
    ...
    println!("\n4️⃣  No Overlap");
    let chunker_no_overlap = Chunker::builder()
        .add_overlap(false)
        .build();
...
fn print_summary(chunks: &[markdown_chunker::Chunk]) {
    ...
    let code_blocks = chunks.iter().filter(|c| c.metadata.chunk_type == ChunkType::CodeBlock).count();
    let tables = chunks.iter().filter(|c| c.metadata.chunk_type == ChunkType::Table).count();
    let lists = chunks.iter().filter(|c| c.metadata.chunk_type == ChunkType::List).count();
