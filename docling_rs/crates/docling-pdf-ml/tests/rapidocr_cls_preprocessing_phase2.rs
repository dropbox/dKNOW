...
use docling_pdf_ml::preprocessing::rapidocr::rapidocr_cls_preprocess;
/// RapidOCR Classification Preprocessing - Phase 2 Validation
///
...
///
/// Related files:
/// - Python baseline: ml_model_inputs/rapid_ocr_isolated/cls_preprocessed_input.npy
/// - Raw cropped boxes: ml_model_inputs/rapid_ocr/cropped_text_box_{i}.npy
/// - Rust implementation: src/preprocessing/rapidocr.rs::rapidocr_cls_preprocess()
///
/// Context:
/// - Detection preprocessing validated at N=83 (max diff 0.0157)
/// - Expected similar performance for classification preprocessing
...
fn test_rapidocr_cls_preprocessing_phase2() {
    println!("\n=== RapidOCR Classification Preprocessing Phase 2 ===");
    println!("Testing: Rust preprocessing vs Python baseline");
    println!("Threshold: max diff < {}\n", MAX_DIFF_THRESHOLD);

    // Load Python baseline (3 preprocessed boxes)
    let baseline_path = "ml_model_inputs/rapid_ocr_isolated/cls_preprocessed_input.npy";
    ...
    for i in 0..3 {
        ...
        let rust_output = rapidocr_cls_preprocess(&crop);
        println!("  Output shape: {:?}", rust_output.dim());
        assert_eq!(
            rust_output.dim(),
            (3, 48, 192),
            "Output should be (3, 48, 192)"
        );
        ...
        if max_diff >= MAX_DIFF_THRESHOLD {
            ...
            for x in (w.saturating_sub(2))..=(w + 2).min(191) {
                println!(
                    "    Rust[{}, {}, {}] = {:.6}, Python = {:.6}",
                    c,
                    h,
    ...
    if overall_max < MAX_DIFF_THRESHOLD {
        println!("\n✓ Phase 2 Classification Preprocessing: PASS");
    } else {
        println!("\n✗ Phase 2 Classification Preprocessing: FAIL");
        panic!(
