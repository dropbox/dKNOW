# SuperGrep (sg) Architecture

Technical design for `sg` - a semantic code search CLI using XTR-WARP multi-vector retrieval.

**Upstream:** [rust-warp](https://github.com/jhansen_dbx/rust-warp) (Rust port of XTR-WARP)
**Reference:** [xtr-warp](https://github.com/jlscheerer/xtr-warp) (Original Stanford implementation)

---

# Part 1: Progressive Semantic Search Indexing
...
// User runs sg search
fn main() {
    let client = IndexerClient::connect(SOCKET_PATH)?;
    let results = client.search(&query, SearchOptions::default())?;
...
fn semantic_search_tool(query: &str, scope: &str) -> Vec<SearchResult> {
    let client = IndexerClient::connect(SOCKET_PATH)?;
...
### For Terminal Integration (Future)
...
1. **Product Quantization** - ✓ Done - 32x compression (512→16 bytes per embedding), asymmetric distance computation
2. **HNSW for navigation** - ✓ Done - O(log n) cluster selection instead of O(n) linear scan
...
| Backend | Platform | Benefit | Status |
|---------|----------|---------|--------|
| **ONNX Runtime** | Cross-platform | Portable, optimized inference | ✓ Done |
| **CoreML** | macOS/iOS | Native Apple Silicon optimization | ✓ Done |
| **CUDA** | NVIDIA GPUs | Fast indexing on Linux/Windows servers | ✓ Done |
| **OpenVINO** | Intel CPUs | 2-3x faster on Intel hardware | ✓ Done |
| **TensorRT** | NVIDIA GPUs | Optimized inference | ✓ Done |

**Implementation path:**
```rust
enum InferenceBackend {
    Candle,      // Done: pure Rust, Metal on macOS
    Onnx,        // Done: ort crate, cross-platform
    CoreMl,      // Done: Native macOS via ort with CoreML execution provider
    Cuda,        // Done: ort crate with CUDA execution provider
    OpenVino,    // Done: ort crate with OpenVINO execution provider
    TensorRt,    // Done: ort crate with TensorRT execution provider
}
...
**Priority:**
1. ~~ONNX Runtime~~ ✓ Done - Best cross-platform story, single export works everywhere
2. ~~CoreML~~ ✓ Done - Better macOS integration via ONNX Runtime CoreML execution provider
3. ~~CUDA~~ ✓ Done - Fast indexing on Linux/Windows GPU servers via ONNX Runtime CUDA execution provider
4. ~~OpenVINO~~ ✓ Done - 2-3x faster on Intel CPUs via ONNX Runtime OpenVINO execution provider
5. ~~TensorRT~~ ✓ Done - Optimized inference on NVIDIA GPUs via ONNX Runtime TensorRT execution provider
...
# Part 2: sg Command & Daemon Integration

## The `sg` Command (SuperGrep)
...
## Architecture: Daemon + CLI Integration
...
│                                                                     │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐              │
│  │  sg search   │  │  sg search   │  │  Agent       │   Clients    │
│  │  (terminal 1)│  │  (terminal 2)│  │  (tool call) │              │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘              │
│         │                 │                 │                       │
│         │    Unix Socket / IPC              │                       │
│         └─────────────────┼─────────────────┘                       │
...
```rust
// In sg CLI - ensure daemon running before any operation
...
├── index.db                      # Main SQLite index database
...
Note: Config parsing is supported for a subset of settings used by the daemon.
Currently read:
- `[resources] max_total_mb` - Maximum total storage (default: 2048 MB)
- `[indexing] stale_project_days` - Days before unused projects are evicted (default: 30)
- `[indexing] idle_threshold_secs` - Seconds before speeding up indexing (default: 300)
...
```
macOS: ~/Library/Caches/sg/models/google_xtr-base-en/
Linux: ~/.cache/sg/models/google_xtr-base-en/
├── config.json                   # Model config
├── tokenizer.json                # Tokenizer data
└── model.safetensors             # Model weights (~100MB)
```
...
Socket:     ~/.local/share/sg/daemon.sock (Linux) or ~/Library/Application Support/sg/daemon.sock (macOS)
Storage:    847 MB / 2048 MB (41%)
Model:      Loaded (XTR-base, 198 MB RAM)

Indexed Projects:
  ● ~/code/myproject      2,847 files   142 MB   active      98%
...
|-----------|------|-------|
| **sg-daemon** | Background daemon | Single process, started by `sg` on-demand |
| **sg-core** | Core library | Embeddings, indexing, search logic |
| **sg** | CLI command | User-facing binary |
| **index.db** | Storage | macOS: `~/Library/Application Support/sg/index.db`, Linux: `~/.local/share/sg/index.db` |
| **model.safetensors** | XTR weights (cached) | macOS: `~/Library/Caches/sg/models/google_xtr-base-en/`, Linux: `~/.cache/sg/models/google_xtr-base-en/` |

The user experience:
1. Install `sg` binary
2. `sg "handle auth"` → daemon starts, project indexed, results returned
3. Background indexing continues as you work
4. Agents can use semantic search via IPC
