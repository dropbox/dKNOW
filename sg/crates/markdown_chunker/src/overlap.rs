...
impl OverlapStrategy {
    ...
    pub fn apply(&self, chunks: Vec<Chunk>) -> Vec<Chunk> {
        ...
        for i in 0..chunks.len() {
            ...
            if i > 0 {
                let prev_suffix =
                    self.get_sentence_suffix(&chunks[i - 1].content, self.overlap_tokens);
                if !prev_suffix.is_empty() {
                    content = format!("{prev_suffix}\n\n{content}");
                }
            }
...
mod tests {
    ...
    fn test_overlap_multiple_chunks() {
        ...
        assert_eq!(
            overlapped[0].content,
            "First chunk. This is sentence one. This is sentence two."
        );
