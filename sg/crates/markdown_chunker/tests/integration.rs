...
fn test_no_content_loss() {
    ...
    let preserved_count = original_words
        .iter()
...
fn test_chunks_within_limits() {
    ...
    for chunk in &chunks {
        ...
        if chunk.metadata.chunk_type == markdown_chunker::ChunkType::Paragraph {
            assert!(
                chunk.metadata.token_count <= max_tokens * 2,
...
fn test_mixed_japanese() {
    ...
    let has_japanese = chunks.iter().any(|c| {
        c.content
            .chars()
            .any(|ch| ('\u{3040}'..='\u{309F}').contains(&ch))
    });
