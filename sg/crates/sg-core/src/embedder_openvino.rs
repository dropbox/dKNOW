//! OpenVINO embedding backend (Intel CPUs)
//!
//! This module provides an OpenVINO-accelerated embedder using ONNX Runtime
//! with the OpenVINO execution provider. It uses the same ONNX model as the
//! standard ONNX backend but runs inference with Intel-specific optimizations
//! for 2-3x faster performance on Intel CPUs.
//!
//! # Requirements
//!
//! - Intel CPU (Haswell or newer recommended)
//! - OpenVINO Runtime installed (2022.1 or newer)
...
//!
//! ```rust,ignore
//! use sg_core::embedder_openvino::OpenVINOEmbedder;
//!
//! let embedder = OpenVINOEmbedder::new()?;
...
use ort::{
    execution_providers::openvino::OpenVINOExecutionProvider, session::Session, value::Value,
};
...
/// OpenVINO-accelerated embedder using ONNX Runtime
...
impl OpenVINOEmbedder {
    /// Load the OpenVINO embedder from the default cache location
    ...
    pub fn from_files(model_path: &Path, tokenizer_path: &Path) -> Result<Self> {
        tracing::info!(
            "Loading ONNX model with OpenVINO acceleration from {}",
            model_path.display(),
        );

        // Initialize ONNX Runtime session with OpenVINO execution provider
        let openvino_provider = OpenVINOExecutionProvider::default().build();

        let session = Session::builder()
            .context("Failed to create ONNX session builder")?
            .with_execution_providers([openvino_provider])
            .context("Failed to configure OpenVINO execution provider")?
        ...
        tracing::info!("OpenVINO embedder loaded successfully");
    ...
    fn embed(&mut self, text: &str, max_len: usize) -> Result<Array2<f32>> {
        ...
        // Run inference (OpenVINO accelerated)
        let outputs = self
            .session
...
/// Implement EmbedderBackend for OpenVINO Embedder
impl crate::embedder::EmbedderBackend for OpenVINOEmbedder {
...
mod tests {
    ...
    fn test_projection_weights_shape() {
        let weights = OpenVINOEmbedder::init_projection_weights();
        assert_eq!(weights.shape(), &[EMBEDDING_DIM, HIDDEN_DIM]);
    }

    #[test]
    fn test_projection_weights_deterministic() {
        let w1 = OpenVINOEmbedder::init_projection_weights();
        let w2 = OpenVINOEmbedder::init_projection_weights();
        assert_eq!(
            w1, w2,
    ...
    /// Compile-time test: verify OpenVINOEmbedder implements EmbedderBackend
    #[allow(dead_code)]
    fn verify_embedder_backend_impl() {
        fn requires_backend<E: crate::embedder::EmbedderBackend>(_: &mut E) {}

        fn _test_openvino_embedder(embedder: &mut OpenVINOEmbedder) {
