//! TensorRT embedding backend (NVIDIA GPUs with optimized inference)
//!
//! This module provides a TensorRT-accelerated embedder using ONNX Runtime
//! with the TensorRT execution provider. TensorRT provides optimized inference
//! performance compared to standard CUDA by applying graph optimizations,
...
//! - TensorRT installed (8.x or later)
//! - cuDNN installed
...
//!
//! ```rust,ignore
//! use sg_core::embedder_tensorrt::TensorRTEmbedder;
//!
//! let embedder = TensorRTEmbedder::new()?;
//! let embeddings = embedder.embed_document("some text")?;
//! ```
//!
//! # Performance
//!
//! TensorRT typically provides 2-4x faster inference than standard CUDA
//! through optimizations like:
//! - Layer fusion (combining multiple operations)
//! - Kernel auto-tuning for specific GPU
//! - FP16/INT8 precision (when available)
//! - Memory optimization
//!
//! Note: First inference may be slower due to TensorRT engine building.

use anyhow::{Context, Result};
use ndarray::{Array2, Axis};
use ort::{
    execution_providers::tensorrt::TensorRTExecutionProvider, session::Session, value::Value,
};
...
/// TensorRT-accelerated embedder using ONNX Runtime
pub struct TensorRTEmbedder {
    session: Session,
    tokenizer: Tokenizer,
    projection_weights: Array2<f32>,
}

impl TensorRTEmbedder {
    /// Load the TensorRT embedder from the default cache location
    pub fn new() -> Result<Self> {
        Self::with_device_id(0)
    }

    /// Load the TensorRT embedder for a specific GPU device
    pub fn with_device_id(device_id: i32) -> Result<Self> {
        let model_dir = Self::default_model_dir()?;
        Self::from_dir_with_device(&model_dir, device_id)
    }
    ...
    pub fn from_dir(model_dir: &Path) -> Result<Self> {
        Self::from_dir_with_device(model_dir, 0)
    }

    /// Load from a specific directory with a specific GPU device
    pub fn from_dir_with_device(model_dir: &Path, device_id: i32) -> Result<Self> {
        ...
        Self::from_files_with_device(&model_path, &tokenizer_path, device_id)
    }

    /// Load from specific file paths
    pub fn from_files(model_path: &Path, tokenizer_path: &Path) -> Result<Self> {
        Self::from_files_with_device(model_path, tokenizer_path, 0)
    }

    /// Load from specific file paths with a specific GPU device
    pub fn from_files_with_device(
        model_path: &Path,
        tokenizer_path: &Path,
        device_id: i32,
    ) -> Result<Self> {
        tracing::info!(
            "Loading ONNX model with TensorRT acceleration from {} (device {})",
            model_path.display(),
            device_id
        );

        // Initialize ONNX Runtime session with TensorRT execution provider
        ...
        tracing::info!(
            "TensorRT embedder loaded successfully on device {}",
            device_id
        );
    ...
    pub fn init_projection_weights() -> Array2<f32> {
    ...
    fn embed(&mut self, text: &str, max_len: usize) -> Result<Array2<f32>> {
        ...
        let outputs = self
            ...
            .context("TensorRT inference failed")?;
...
/// Implement EmbedderBackend for TensorRT Embedder
impl crate::embedder::EmbedderBackend for TensorRTEmbedder {
...
mod tests {
    ...
    fn test_projection_weights_shape() {
        let weights = TensorRTEmbedder::init_projection_weights();
        assert_eq!(weights.shape(), &[EMBEDDING_DIM, HIDDEN_DIM]);
    }

    #[test]
    fn test_projection_weights_deterministic() {
        let w1 = TensorRTEmbedder::init_projection_weights();
        let w2 = TensorRTEmbedder::init_projection_weights();
        assert_eq!(
            w1, w2,
    ...
    // Note: Cross-module projection weight comparison tests removed.
    ...
    /// Compile-time test: verify TensorRTEmbedder implements EmbedderBackend
    #[allow(dead_code)]
    fn verify_embedder_backend_impl() {
        fn requires_backend<E: crate::embedder::EmbedderBackend>(_: &mut E) {}

        fn _test_tensorrt_embedder(embedder: &mut TensorRTEmbedder) {
