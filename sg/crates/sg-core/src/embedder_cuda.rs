//! CUDA embedding backend (NVIDIA GPUs)
//!
//! This module provides a CUDA-accelerated embedder using ONNX Runtime
//! with the CUDA execution provider. It uses the same ONNX model as the standard
//! ONNX backend but runs inference on NVIDIA GPUs for faster indexing.
//!
//! # Requirements
//!
...
//!
//! ```rust,ignore
//! use sg_core::embedder_cuda::CUDAEmbedder;
//!
...
use ort::{execution_providers::cuda::CUDAExecutionProvider, session::Session, value::Value};
...
/// CUDA-accelerated embedder using ONNX Runtime
...
impl CUDAEmbedder {
    /// Load the CUDA embedder from the default cache location
    pub fn new() -> Result<Self> {
        Self::with_device_id(0)
    }

    /// Load the CUDA embedder for a specific GPU device
    ...
    pub fn from_files_with_device(
        ...
        tracing::info!(
            "Loading ONNX model with CUDA acceleration from {} (device {})",
            model_path.display(),
            device_id
        );
        ...
        tracing::info!("CUDA embedder loaded successfully on device {}", device_id);
...
/// Implement EmbedderBackend for CUDA Embedder
impl crate::embedder::EmbedderBackend for CUDAEmbedder {
...
mod tests {
    ...
    fn test_projection_weights_shape() {
        let weights = CUDAEmbedder::init_projection_weights();
        assert_eq!(weights.shape(), &[EMBEDDING_DIM, HIDDEN_DIM]);
    }

    #[test]
    fn test_projection_weights_deterministic() {
        let w1 = CUDAEmbedder::init_projection_weights();
        let w2 = CUDAEmbedder::init_projection_weights();
        assert_eq!(
            w1, w2,
    ...
    /// Compile-time test: verify CUDAEmbedder implements EmbedderBackend
    #[allow(dead_code)]
    fn verify_embedder_backend_impl() {
        fn requires_backend<E: crate::embedder::EmbedderBackend>(_: &mut E) {}

        fn _test_cuda_embedder(embedder: &mut CUDAEmbedder) {
